  ...
  <div class="p-4 sm:p-6">
    ...
    <template v-else-if="summary">
      ...
      <div class="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 mb-6">
        ...
        <div v-if="comparison" class="mt-3 grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs text-gray-500">
          <span>Réels : <strong class="text-gray-700">{{ fmt(comparison.realExpensesTotal) }}</strong></span>
          <span>Forfait : <strong class="text-gray-700">{{ fmt(comparison.standardDeduction) }}</strong> (plafond {{ fmt(comparison.standardDeductionCap) }})</span>
          <span>Année {{ comparison.year }} · taux {{ comparison.standardDeductionRate * 100 }} %</span>
        </div>
        <ul v-if="comparison" class="mt-3 space-y-1 text-xs text-amber-700" aria-label="Avertissements fiscaux">
          <li v-for="warning in comparison.warnings" :key="warning">⚠️ {{ warning }}</li>
        </ul>
      </div>
