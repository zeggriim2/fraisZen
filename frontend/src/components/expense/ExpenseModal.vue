  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="$emit('close')">
    ...
    <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col">
      ...
      <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200">
        <div>
          <h2 class="text-lg font-semibold text-gray-900">{{ expense && !editing ? 'Détail' : 'Ajouter un frais' }}</h2>
      ...
      <div v-if="expense && !editing" class="flex-1 overflow-y-auto px-6 py-5 space-y-1">
        ...
        <InfoRow v-if="expense.type === 'toll'" label="Départ" :value="(expense as TollExpense).departure ?? '—'" />
        <InfoRow v-if="expense.type === 'toll'" label="Arrivée" :value="(expense as TollExpense).arrival ?? '—'" />
        <InfoRow v-if="expense.type === 'toll'" label="Montant" :value="`${(expense as TollExpense).tollAmount.toFixed(2)} €`" />
        <InfoRow v-if="expense.type === 'remote_work'" label="Indemnité" value="2,50 €" />
      ...
      <div v-else class="flex-1 overflow-y-auto px-6 py-5 space-y-5">
        ...
        <template v-if="form.type === 'travel'">
          ...
          <div class="flex items-center justify-between p-3 rounded-xl border border-gray-200 bg-gray-50">
            <div>
              <p class="text-sm font-medium text-gray-700">Aller-retour</p>
              <p class="text-xs text-gray-500 mt-0.5">La distance saisie sera multipliée par 2</p>
            </div>
          ...
          <!-- Electric toggle — voitures uniquement -->
...
import type { Expense, TravelExpense, TollExpense, VehicleType } from '@/types'
import InfoRow from '@/components/ui/InfoRow.vue'

const props = defineProps<{ date: string; expense?: Expense | null }>()
...
const form = ref({
  type: (props.expense?.type ?? 'travel') as 'travel' | 'remote_work' | 'toll',
  ...
  try {
    ...
    if (form.value.type === 'travel') {
      await expenseStore.create({
        ...
        vehiclePower: form.value.vehicleType !== 'moped' ? form.value.vehiclePower : undefined,
        isElectric: form.value.vehicleType === 'car' ? form.value.isElectric : undefined,
        departure: form.value.departure || undefined,
        arrival: form.value.arrival || undefined,
